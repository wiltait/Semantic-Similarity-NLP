...
# and check out other cool stuff!
    ...
    Here I ran both cases using the models 'en_core_web_sm' and 'en_core_web_md'
    There is a difference in the levels of similarities that the models find.
    the 'md' is able to pick up a higher level of similarity than the smaller model 'sm'
    When I ran the model 'en_core_web_sm', I get the following warning:
    ...
    UserWarning: [W007] The model you're using has no word vectors loaded, so the result of the Doc.similarity method will be based on the tagger,
      ...
      which don't ship with word vectors and only use context-sensitive tensors. You can always add your own word vectors, or use one of the larger 
      models instead if available.   

    According to the official documentation spaCy uses small models to make them compact and fast, which "don't ship with word vectors, 
    and only include context-sensitive tensors." You can still use the similarity() methods to compare documents with small models as 
    well but "the result won't be as good, and individual tokens won't have any vectors assigned.
